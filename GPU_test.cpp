...
int main(int argc, char *argv[])
{
	...
	//const char *zInput = argc > 1 ? argv[1] : "./car10.jpg";
	...
	float *data;
	float buff;
	data = (float*)malloc(1000 * 277);

	fp = fopen("zInput.txt", "r");
	fp_o = fopen("outp.txt", "w+");
	for (int i = 0; i < 182; i++) {
		for (int j = 0; j < 277; j++)
		{
	...
	*/
	...
	for (int i = 0; i < 182; i++) {
		for (int j = 0; j < 277; j++)
		{
			fprintf(fp_out, "%d ", (int)imgIn.data[i*j]);
		}
		fprintf(fp_out, " \n");
	}
	fclose(fp_out);
